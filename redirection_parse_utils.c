...
/*   Updated: 2022/04/12 12:47:34 by ikgonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_check_red_out_del(t_proc *proc, int i, int *j, int red_out_del_arr_len)
{
    *j += 1;
    if (red_out_del_arr_len < proc->red_out_del_count
    && proc->red_out_del_arr[red_out_del_arr_len++] == 1)
    {
...
{
    if (red_in_arr_len < proc->red_in_count && proc->red_in_arr[red_in_arr_len++] == 1)
    {
        if (proc->process[i][*j - 1] != 32 || proc->process[i][*j - 1] != 126)
            proc->added_spc++;
        if (proc->process[i][*j + 1] != 32 || proc->process[i][*j + 1] != 126)
